...
# Usage: bash transcribe.sh <Xiaoyuzhou URL> [output file path]
# Environment variable: GROQ_API_KEY (required)

set -e

URL="${1:?Usage: bash transcribe.sh <Xiaoyuzhou URL> [output file path]}"
...
if [ -z "$AUDIO_URL" ]; then
    echo "❌ Could not extract the audio link from the page"
    exit 1
fi
...

{
    ...
    echo "Transcription time: $(date '+%Y-%m-%d %H:%M')"
    echo ""
    echo "---"
    echo ""
