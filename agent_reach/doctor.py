...
def format_report(results: Dict[str, dict]) -> str:
    ...
    if tier1:
        lines.append("")
        lines.append("[bold]Search (unlocked by mcporter):[/bold]")
    ...
    lines.append("")
    status_color = (
        "green" if ok_count == total else ("yellow" if ok_count > 0 else "red")
    )
    lines.append(
        f"Status: [{status_color}]{ok_count}/{total}[/{status_color}] channels available"
    )
    if ok_count < total:
