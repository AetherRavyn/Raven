# -*- coding: utf-8 -*-
"""
Agent Reach CLI - installer, health checks, and configuration tool.
...
def main():
    ...
    parser.add_argument(
        "--version", action="version", version=f"Agent Reach v{__version__}"
    )
    ...
    p_conf = sub.add_parser(
        "configure", help="Set a config value or auto-extract from browser"
    )
    p_conf.add_argument(
        "key",
        nargs="?",
    ...
    # ── watch ──
    sub.add_parser(
        "watch", help="Quick health check + update check (for scheduled tasks)"
    )
...
def _cmd_install(args):
    ...
    if env == "local" and not safe_mode and not dry_run:
        ...
        try:
            ...
            if not found:
                print(
                    "  -- No cookies found (normal if you haven't logged into these sites)"
                )
        except Exception:
    ...
    if env == "server":
        ...
        print(
            "   For full access: agent-reach configure proxy http://user:pass@ip:port"
        )
    ...
    if not dry_run:
        ...
        print()
        print(
            "If Agent Reach helped you, please give it a star so more people can discover it:"
        )
        print("   https://github.com/Panniantong/Agent-Reach")
        print(
            "   It only takes a second and means a lot to an independent developer. Thank you!"
        )
    else:
        print()
...
def _uninstall_skill():
    ...
    if openclaw_home:
        skill_dirs.insert(
            0,
            (
                os.path.join(openclaw_home, ".openclaw", "skills", "agent-reach"),
                "OpenClaw",
            ),
        )
    ...
    if shutil.which("gh"):
        ...
        if os_type == "linux":
            ...
            if shutil.which("brew"):
                try:
                    subprocess.run(
                        ["brew", "install", "gh"], capture_output=True, timeout=120
                    )
                    if shutil.which("gh"):
                        print("  ✅ gh CLI installed")
                    else:
    ...
    if npm_cmd:
        ...
        if os.path.exists(undici_path):
            ...
            try:
                ...
                print(
                    "  -- undici install failed (optional — bird may not work behind proxies)"
                )
    ...
    if shutil.which("node"):
        ...
        if needs_config:
            try:
                ...
                print(
                    "  -- Could not configure yt-dlp JS runtime (YouTube may not work)"
                )
...
def _install_xiaoyuzhou_deps():
    ...
    if os.path.isfile(script_dst):
        ...
        script_src = os.path.join(
            os.path.dirname(__file__), "scripts", "transcribe_xiaoyuzhou.sh"
        )
    ...
    if shutil.which("ffmpeg"):
        ...
        print(
            "  -- ffmpeg not found. Install: apt install -y ffmpeg (or brew install ffmpeg)"
        )
...
def _install_wechat_deps():
    ...
    if has_camoufox and has_miku:
        ...
        try:
            ...
            print(
                f"  [!]  WeChat packages install failed. Try: pip install {' '.join(pkgs)}"
            )
...
def _install_system_deps_safe():
    ...
    deps = [
        ...
        (
            ...
            ["node", "npm"],
            "Node.js",
            "https://nodejs.org — or: apt install nodejs npm",
        ),
        (
            "bird",
            ["bird", "birdx"],
    ...
    try:
        ...
        if "xiaohongshu" in r.stdout:
            ...
            try:
                ...
                print(
                    "     Then:    mcporter config add xiaohongshu http://localhost:18060/mcp"
                )
...
def _install_mcporter_safe():
    ...
    if shutil.which("mcporter"):
        print("  ✅ mcporter already installed")
        print(
            "  To configure Exa search: mcporter config add exa https://mcp.exa.ai/mcp"
        )
    else:
...
def _cmd_configure(args):
    ...
    if args.from_browser:
        ...
        if found_any:
            ...
            print(
                f"No cookies found. Make sure you're logged into the platforms in {browser}."
            )
        return
    ...
    if args.key == "proxy":
        ...
        if auth_token and ct0:
            ...
            try:
                ...
                if not bird:
                    print(
                        "[!] bird CLI not installed. Run: npm install -g @steipete/bird"
                    )
                else:
                    import os
            ...
            print(
                '   2. agent-reach configure twitter-cookies "auth_token=xxx; ct0=yyy; ..."'
            )
        ...
        print(
            "   yt-dlp will use cookies from this browser for age-restricted/member videos."
        )
...
def _configure_xhs_cookies(value):
    ...
    if not value:
        ...
        print(
            "   Usage: agent-reach configure xhs-cookies '<cookie JSON or header string>'"
        )
        return
        ...
        try:
            ...
            if isinstance(parsed, list) and parsed:
                ...
                if isinstance(first, dict) and "name" in first and "value" in first:
                    ...
                    print(
                        "[X] JSON array doesn't contain cookie objects (need name/value fields)"
                    )
                    return
            else:
    ...
    if not cookies_json:
        ...
        print(
            '   1. JSON array: \'[{"name":"x","value":"y","domain":".xiaohongshu.com",...}]\''
        )
    ...
    try:
        ...
        if not cookie_path_in_container:
            cookie_path_in_container = (
                "/app/cookies.json"  # fallback: absolute path in workdir
            )
    except Exception:
    ...
    if mcporter:
        ...
        try:
            ...
            if (
                ...
                print(
                    "  Cookies were written but login might not be valid. Try fresh cookies."
                )
        except Exception as e:
    ...
    if not keep_config:
        if os.path.isdir(config_dir):
            if dry_run:
                ...
                print(
                    "          (contains config.yaml with all tokens/cookies/API keys)"
                )
            else:
                try:
...
def _cmd_setup():
    ...
    if not shutil.which("mcporter"):
        ...
        try:
            ...
            if "exa" in r.stdout.lower():
                ...
                setup_now = (
                    input("  Configure Exa automatically now? [Y/n]: ").strip().lower()
                )
            ...
            print("  [!] Could not check Exa configuration. Run manually:")
            print("     mcporter config add exa https://mcp.exa.ai/mcp")
        print()

    # Step 2: GitHub token
    print("[Optional] GitHub token — increase API rate limits")
    print("  No token: 60 requests/hour | With token: 5000 requests/hour")
    print("  Get one at: https://github.com/settings/tokens (no permissions required)")
    ...
    if current:
        ...
        if key:
            ...
            print("  ✅ GitHub API rate limit increased to 5000 requests/hour!")
        else:
            print("  Skipped. The public API still works")
    print()

    # Step 3: Reddit proxy
    print("[Optional] Reddit proxy — read full Reddit posts and comments")
    print("  Reddit blocks many IPs, so an ISP proxy is required for direct access")
    ...
    if current:
        ...
        if proxy:
            ...
            print("  ✅ Full Reddit reading is enabled!")
        else:
            print("  Skipped. Reddit content is still available through search")
    print()

    # Step 4: Groq (Whisper)
    print("[Optional] Groq API — speech-to-text when videos have no subtitles")
    print("  Free tier available, sign up at: https://console.groq.com")
    ...
    if current:
        ...
        if key:
            ...
            print("  ✅ Speech-to-text is enabled!")
        else:
            print("  Skipped")
    print()
...
def _cmd_check_update():
    ...
    if err:
        print(
            f"[!] Could not check for updates ({_update_error_text(err)}, retried {attempts} times)"
        )
        return "error"
    ...
    if release_err == "rate_limit":
        print(
            "[!] Could not check for updates (GitHub API rate limit, please try again later)"
        )
        return "error"
    ...
    if err2:
        print(
            f"[!] Could not check for updates ({_update_error_text(err2)}, retried {attempts + attempts2} times)"
        )
        return "error"
...
def _cmd_watch():
    ...
    if not issues and not update_available:
        print(
            f"Agent Reach: all good ({ok}/{total} channels available, v{__version__} is up to date)"
        )
        return
    ...
    if update_available:
        ...
        if release_body:
            for line in release_body.strip().split("\n")[:10]:
                print(f"    {line}")
        print(
            f"  Update: pip install --upgrade https://github.com/Panniantong/agent-reach/archive/main.zip"
        )
