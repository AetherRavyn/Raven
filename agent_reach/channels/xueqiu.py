# -*- coding: utf-8 -*-
"""Xueqiu — stock quotes, search, trending posts, and hot stocks."""
...
class XueqiuChannel(Channel):
    ...
    def check(self, config=None):
        try:
            ...
            if items:
                return (
                    "ok",
                    "Public API is available (quotes, search, hot posts, hot stocks)",
                )
            return "warn", "Unexpected API response (empty data returned)"
        except Exception as e:
            return (
                "warn",
                f"Xueqiu API connection failed (a proxy may be required): {e}",
            )
    ...
    def get_stock_quote(self, symbol: str) -> dict:
        ...
        Args:
            symbol: Stock code, such as SH600519, SZ000858, AAPL, or 00700
    ...
    def search_stock(self, query: str, limit: int = 10) -> list:
        ...
        Args:
            query: Stock code or name, such as "Maotai" or "600519"
    ...
    def get_hot_posts(self, limit: int = 20) -> list:
        ...
        for item in items[:limit]:
            ...
            text = _strip_html(
                original.get("text") or original.get("description") or ""
            )
